-- 0029 — tighten the smart_pending cohort: it must mean "chose Smart but has NOT scanned yet"
-- (ADR-0011). So require smart + pending AND zero earning scans, robust to substate drift.
    ...
    select pid, em, dn, st, ct, ca, count(*) over()
     ...
     where (p_query is null or em ilike '%' || p_query || '%' or coalesce(dn, '') ilike '%' || p_query || '%')
       ...
       and (p_flag is null
            ...
            or (p_flag = 'smart_pending' and rm = 'smart' and ss = 'pending' and not has_scans))
     order by ca desc
