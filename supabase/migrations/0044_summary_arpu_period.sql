-- 0044 — make Summary ARPU actually respect the selected period (it was all-time, which made it
-- look "broken" when changing the date filter). ARPU = revenue captured IN the period / distinct
-- payers IN the period. Everything else in fn_admin_summary is unchanged from 0036.
    ...
    'kpis', jsonb_build_object(
      ...
      'arpu', coalesce((select sum(o.amount) from orders o join subscriptions s on s.id=o.subscription_id
              ...
              / nullif((select count(distinct s.owner_profile_id) from orders o join subscriptions s on s.id=o.subscription_id
                         where o.charge_status='captured' and o.paid_at between p_from and p_to and s.owner_profile_id in (select id from scope)), 0)
    ),
