-- 0025 — Overview upgrades: (1) accept an explicit date RANGE (p_from/p_to) so the UI can offer
-- today / yesterday / this month / a specific month / custom, and (2) add per-referrer earnings
-- (top_referrers: each Agent/Affiliate with paid + pending + total commission and active subs).
drop function if exists fn_admin_overview(integer, text);
...
begin
  ...
  select jsonb_build_object(
    ...
    'top_referrers', coalesce((
      ...
      from referrers r join auth.users u on u.id = r.profile_id
    ), '[]'::jsonb),
