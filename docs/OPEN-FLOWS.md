...
| ✅ Gamification & referral config made Admin-dynamic; per-dial grandfathering; checkpoint XP; margin human-managed (calculator), not enforced | [ADR-0013](./adr/0013-dynamic-config-grandfathering-and-manual-margin.md), [CONTEXT](../CONTEXT.md), [admin-console §8](./product/admin-console.md#8-gamification--referral-config-) |
...
- ⬜ **Pricing & Promotions track (agent tiers + FluoFit promos) — dedicated grill+build.** (founder, 2026-08).
  Three discount sources: (1) **agent tier → {commission %, buyer discount %} + threshold** (tier table; the
  auto tier-recompute + rates wait on financial modelling — currently manual `fn_admin_set_tier`); (2)
  **affiliate** manual commission%+discount% (DONE, Affiliates tab); (3) **FluoFit's own time-bound promos**
  (NET-NEW — period-scoped campaign discounts, independent of referral; allowed since ADR-0002 only forbids
  *Level*-funded price cuts, not marketing promos). Open rule: **do promo + referral discounts stack?**
  (likely take-the-best, non-stacking). Home = a "Pricing & Promotions" tab (the current minimal `/settings`
  holds the pricing/referral dials as its seed). Blocked partly on pending financial numbers.
- ⬜ **Perk financial engine (spend-perk cost into margin).** ADR-0002 two-purse: spend-funded perks are a
  real cost snapshot-at-crossing. Infra exists (`member_reward_snapshots`, `partner_perks`) but nothing writes
  to it and the Financial margin (`fn_admin_overview`) ignores perk cost entirely. Gamification now shows an
  *estimate* (reach × cost_hint). A real engine would: on level-crossing snapshot actual spend-perk cost →
  `member_reward_snapshots` → fold into Financial margin. Needs the level-crossing/reward engine + real perk
  grants (perks aren't granted anywhere in v1). Deferred to the economics/financial-modelling track.
- ⬜ **City normalisation (Google Places) + what a "city"/region actually is** — **near-term priority** (founder, 2026-08). [ADR-0016](./adr/0016-delivery-address-and-city.md) already **decided** Google Places Autocomplete for the delivery city, but it is **not yet implemented**: city inputs are still **free-text** everywhere one is entered (intake-wave `city_focus`, add-affiliate, seed data). This already bit us — a wave was created for `belgrade` instead of `beograd`, so the hard city gate would silently match no one. **Immediate need:** replace every free-text city field with a **controlled Places-backed dropdown/autocomplete** (no arbitrary typing) so `city_focus`, member `ship_city`, and any targeting all speak the same canonical value. **Bigger open question (future discussion):** *define what a "city" is* — a Places locality? how large a catchment/region? Should FluoFit instead define **internal regions** that members fall into (e.g. Beograd metro incl. suburbs) so waves/targeting operate on regions, not raw city strings? Undecided — flagged for a dedicated design pass. Depends on / extends [ADR-0016](./adr/0016-delivery-address-and-city.md); ties to intake city-gating ([agent-affiliate-program §1](./product/agent-affiliate-program.md)).

- ⬜ **Unscanned-sachet deactivation vs earning-after-lapse** — founder rule: *sachets not scanned within 60 days are deactivated (can no longer earn XP)*. This is largely the **Benefit clock** ([ADR-0011](./adr/0011-refill-mode-decoupled-and-benefit-clock.md), anti-gaming: one Box ≤ 60 days of live benefits) and does **not** break the fraud floor ([ADR-0006](./adr/0006-aggregate-supply-and-fraud-floor.md)) — it only tightens the earnable pool. **But it clashes with [ADR-0005](./adr/0005-subscription-lifecycle-and-lapse.md) rule (A)** (*"a Lapsed Member keeps earning XP/Streak on already-paid Sachets they hold"*); ADR-0011 already leans to "XP/Level frozen", so ADR-0005↔0011 must be reconciled. **Also undecided:** keying — per **paid-order clock** (documented benefit clock, per-subscription) vs per **Box** (each Box's 60-day window, stricter/new). **Not yet enforced in code** (`fn_sync_scans` ignores time). **Current approach (chosen): informational TAG only, no behaviour change** — the admin UI tags unscanned sachets as *"aged"* (past the 60-day window) vs *"within window"* and shows Received / Scanned / Not-scanned / Adherence % / avg per box. This gives visibility **without** touching the model, so **no conflict** with ADR-0005. Whether aged sachets are ever *actually* deactivated (and the keying — paid-order clock vs per-Box) is a **deferred ADR decision**; nothing is enforced in `fn_sync_scans` yet.
...
- 🟡 **All referral numbers** (discount %, tier %s + thresholds, budget, bonus) — *values* still blocked on COGS → price (financial-modeling pass). **Config *mechanism* now decided:** all are Admin-dynamic dials with per-dial grandfathering (discount → new buyers only; tier → live at next snapshot), margin human-managed via a calculator, not enforced — [ADR-0013](./adr/0013-dynamic-config-grandfathering-and-manual-margin.md).
