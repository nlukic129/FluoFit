...
- **Anti-duplication (software):** one-time activation — once a Box is scanned it can never be scanned again; a second scanner hits the "already bound → contact support" path. **The first scan of a Subscription Box transfers the *whole* Subscription onto the scanner** (billing, refills, notifications, XP/Streak/Level, `ref` — [ADR-0012](./adr/0012-identity-checkout-and-box-ownership.md)), consolidating a web-checkout account and an app-login account into **one** (no split, no linking). A **retail Box** has no Subscription to transfer, so its scanner gets a **Standalone Box** ([ADR-0007](./adr/0007-standalone-gift-retail-box-activation.md)). Trade-off: whoever first scans a delivered Box takes over its Subscription — guarded by the tamper-sealed QR + shipping address + support override.
